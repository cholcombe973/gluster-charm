...
impl Filesystem{
    pub fn new(name: &str)-> Filesystem{
        match name.trim() {
            ...
            _ => Filesystem::Xfs{inode_size: None, force: false},
        }
    }
}
...
pub fn mount_device(device: &Device, mount_point: &str)->Result<i32, String>{
    ...
    match device.id{
        Some(id) => {
            ...
            arg_list.push(id.to_hyphenated_string());
        },
        None => {},
    };
    /*
...
pub fn is_block_device(device_path: &PathBuf)->Result<bool, String>{
    ...
    let sysname = try!(device_path.file_name().ok_or(
        format!("Unable to get file_name on device {:?}", device_path)));
